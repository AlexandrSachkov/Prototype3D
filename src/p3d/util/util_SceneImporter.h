...
namespace p3d {
	namespace util {
		...
		private:
            ...
            static bool loadMeshes(aiMesh** meshes, unsigned int numMeshes, std::vector<model::Mesh>& out);
            static bool loadVertices(aiMesh* mesh, model::Mesh& out);
            static bool loadIndices(aiMesh* mesh, model::Mesh& out);
