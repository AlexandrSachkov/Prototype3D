...
bool run() {
    ...
    meshDesc.vertices = std::shared_ptr<glm::vec3>(new glm::vec3[3]);
    meshDesc.verticesSize = (unsigned int)vertices.size();
    std::memcpy(meshDesc.vertices.get(), vertices.data(), vertices.size() * sizeof(glm::vec3));

    p3d::HMesh hmesh = scene->create(meshDesc);

    p3d::ModelDesc modelDesc;
    modelDesc.mesh = hmesh;

    scene->create(modelDesc);

    sampleRunner.setRunProcedure([&]() {
        renderer->renderFrame(scene.get(), camera.get());
    });
    sampleRunner.start();
